...
public class JdbcPlayerRepository {
	...
	public void save(Player player) {
		jdbcTemplate.update("insert into store(name, gk, rb, cb1, cb2, lb, rw, cm1, cm2, lw, st1, st2, done) values(?,?,?,?,?,?,?,?,?,?,?,?,?)",
				player.getName(), player.getGk(), player.getRb(), player.getCb1(), player.getCb2(), player.getLb(),player.getRw(), player.getCm1(), player.getCm2(), player.getLw(), player.getSt1(), player.getSt2(), player.isDone());
	}
	
	public Player get(int id) {
		return jdbcTemplate.queryForObject(
				"select id, name, gk, rb, cb1, cb2, lb, rw, cm1, cm2, lw, st1, st2, done from store where id=?", new PlayerMapper(),
				id);
	}
		
		public List<Player> getAll() {
			return jdbcTemplate.query("select id, name, gk, rb, cb1, cb2, lb, rw, cm1, cm2, lw, st1, st2, done from store",
					new PlayerMapper());
		}
